...
void image_refinement(const Mat* costVolume, Mat &result_img) 
{
	...
	try {
		...
		show_colorized_image(result_img, "矫正后");
		delete gc;
	}
	catch (GCException e) 
	{
...
void calculate_costvolume(Mat* imgStack, Mat* costVolume) 
{
	...
	//仅作调试用
	show_colorized_image(depthMap, "初始深度图");
	cout << "完成!" << endl;
}
...
void align_images(Mat* imgStack, Mat* resultStack) 
{
	...
	show_colorized_image(imgStack[0] - imgStack[29], "矫正前");
	show_colorized_image(imgStack[0] - resultStack[29], "矫正后");
	cout << "完成!" << endl;
}
